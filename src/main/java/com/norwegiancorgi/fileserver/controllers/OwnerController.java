...
public class OwnerController {
     ...
     */
    @GetMapping
    public ResponseEntity<Owner> getUser(@RequestParam("email") String email) {
        try {
            return ResponseEntity.status(HttpStatus.OK).body(ownerService.getOwnerByEmail(email));
        } catch (Exception e) {
     ...
     */
    @PostMapping
    public ResponseEntity<String> createNewUser(@RequestParam("ownername") String ownername,
                                                @RequestParam("email") String email,
                                                @RequestParam("password") String password) {
        try {
            ownerService.createOwner(ownername, email, password);
     ...
     */
    @DeleteMapping
    public ResponseEntity<String> deleteUser(@RequestParam("id") Long id, @RequestParam("password") String password) {
        try {
            ownerService.deleteUser(id, password);
            return ResponseEntity.status(HttpStatus.OK).body("Owner deleted successfully");
        } catch (Exception e) {
            return ResponseEntity.status(HttpStatus.BAD_REQUEST).body("Owner could not be deleted");
        }
    }
}
